...
airport most relevant to current overhead traffic. Output can be:
  ...
  - browser  : a browser client plays the LiveATC URL itself (zero Pi cost).
               NOTE: this fork has no /display browser mirror, so "browser"
               is hidden in the web UI; the mode still works via the
               stream_url in /api/atc/status if a client wants it.
...
class ATCAudioManager:
    ...
    def _refresh_config(self):
        try:
            ...
            # Port the Flask web app (and thus the loopback relay) listens on.
            # 8080 under systemd; the Docker image runs it on 6969.
            try:
                self._relay_port = int(getattr(cfg, "ATC_RELAY_PORT", 8080) or 8080)
            except (TypeError, ValueError):
                self._relay_port = 8080
    ...
    def _start_cast_locked(self):
        ...
        volume = self._volume
        output = self._output
        relay_port = getattr(self, "_relay_port", 8080)
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        s.close()
                    play_url = (f"http://{my_ip}:{relay_port}/atc/relay"
    ...
    def _start_airplay_locked(self):
        ...
        url = (f"http://127.0.0.1:{getattr(self, '_relay_port', 8080)}"
               f"/atc/relay?code={self._station}"
