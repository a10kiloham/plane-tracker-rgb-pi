...
def get_nearest_city(latitude, longitude):
    ...
    PERFORMANCE: this runs on the RENDER THREAD. scenes/trackedstats.py calls it
    whenever the tracked aircraft moves past _CITY_CACHE_THRESHOLD (0.01 deg, ~1km)
    — at cruise, roughly every 4 seconds. A haversine over all ~69,000 cities
    measured 437ms on a Pi 3A+ against a 100ms frame budget, so each of those
    lookups is a visible scroll freeze.
